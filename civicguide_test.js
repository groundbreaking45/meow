...
test('Google: Places Autocomplete is guarded by mapsInitialized flag', () => {
  let mapsInit = false;
  function safeInitPlaces() { return mapsInit && typeof window.google !== 'undefined'; }
  assert(!safeInitPlaces(), 'Places must not init without mapsInitialized');
  mapsInit = true;
  // Without google global, still false (correct guard)
  assert(!safeInitPlaces(), 'Places must also require window.google');
});
